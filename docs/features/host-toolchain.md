...
The host release matrix targets macOS arm64, Linux x64/arm64, and Windows x64 browser-pack publication; macOS on Intel CPUs is explicitly unsupported; Windows arm64 remains a possible future target. See [`../reference/host-mode-support.md`](../reference/host-mode-support.md) for the platform table and release gates. Future release claims require every release-supported artifact to be published to GitHub Releases and verified with `pnpm --filter @sero/desktop browser-pack:verify-published`. The pack lives below the same fixed toolchain root, under `~/.sero-ui/toolchains/<manifest-version>/browser/`, with its own `.installed` marker. It includes pinned browser/driver/ffmpeg metadata and platform-specific executable candidates.
