...
Before merging the default-on release:

- the full typecheck and Room test suite pass;
- the docs-site release pass is complete;
...
1. Use the default-enabled Rooms for real work, not only the four scripted
...
4. Keep `SERO_ROOMS=0` as a permanent kill switch. Remove the switch only if it
   becomes genuinely unnecessary to keep — not on a fixed schedule.
...
1. Set `SERO_ROOMS=0` and restart Sero.
