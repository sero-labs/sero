...
**Gallery previews.** A version stores a bounded, immutable visual preview for cards and revision selection. Prefer Sero's native app capture: crop the rendered Design preview and reduce it to a useful card-sized PNG. This is not the exported output, so output resolution is unnecessary. Gallery previews stay local and inert; they do not run generated scripts or load remote resources. The exact mechanism is deliberately flexible, and a safe fallback is valid when native capture is unavailable. A thin generic host capture capability is valid when it benefits other plugins; Gallery domain logic stays in the plugin. Do not add a headless-browser dependency or a Design Library-specific host API.
