...
- **The standalone page has no preview harness.** Saved Tweak values are root-element custom properties, so generated CSS cannot replace them through source order. React, Tailwind and approved dependencies are bundled. A local-only CSP remains, but there is no Sero message channel or Tweaks runtime.
...
- **A workspace export has one named parent folder.** It writes `<workspace>/<design-name>/` instead of adding a `design-library-exports/` wrapper. A later export of that Design replaces only a folder that has a valid Design Library export manifest. A compact completion notification opens Downloads in the system file manager or the workspace result in Sero Explorer.
