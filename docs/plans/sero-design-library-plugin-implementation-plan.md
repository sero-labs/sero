...
- **The typography baseline is the one fixed part of Tweaks.** Font, H1 size/weight/tracking, H2 size and Body font/size must be the first controls in every manifest. The generation contract requires the matching custom properties, rules that target their intended text and the two available system font stacks. Qualified selectors such as `.hero h1` are valid. Repair gets a chance to fix omissions. The runtime never rewrites a generated page to add them.
