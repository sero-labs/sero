...
3. **Route UI mutations through explicit extension/host actions instead of raw state writes.**
   - Target structure:
...
6. **Deduplicate config loading and bridge typing once the truthfulness fixes are in.**
   - Add one config loader module for `web-search.json` and one shared host-bridge type import path.
