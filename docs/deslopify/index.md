...
- [`apps/desktop/`](./apps/desktop/plan.md)
  ...
  - [`apps/desktop/src/components/layout/`](./apps/desktop/src/components/layout/plan.md)
    — *In progress — shared git-remote workflow, WorkspaceTree split, ThemeEditorSheet split, ModelSelector split, ContextEditor split, LocalProviderForm split, shared autocomplete/listbox primitive, render-phase side-effect cleanup, and the remaining AuthLoginViews/ToolCallHelpers/ModelManagerDialog near-cap cleanup all cleared 2026-04-15; only ownership repartition Medium work plus Low error-surface follow-up remain*
