...
2. **Move Kanban shared contracts to a neutral shared module.**
   - Target structure: a renderer-safe shared Kanban contract module under `@sero-ai/common` (or an equivalent neutral shared package) that owns:
...
## Next Steps
1. Low-only follow-up: remove or formally land the dead specialized-review scaffolding (`buildQualityReviewPrompt()`) and the currently test-only `core/wave-resolver.ts`.
2. Keep the existing verification checklist for any future Kanban runtime work:
   - Run a card end-to-end through planning → implementation → review → PR creation after workflow changes.
   - Exercise settings permutations (`autoAdvance`, `testingEnabled`, `reviewMode`, `yoloMode`, `yoloAutoMergePrs`) and confirm the runtime behavior matches the exposed settings.
