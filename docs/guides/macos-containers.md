...
On Apple Silicon Macs, Sero works best with Apple's `container` runtime. Across platforms, Sero can also use Docker/Podman-backed workspaces; see [`docs/features/docker-runtime.md`](../features/docker-runtime.md). Containers are **strongly recommended**, but Sero can still continue in reduced host mode on macOS/Linux if they are unavailable.
