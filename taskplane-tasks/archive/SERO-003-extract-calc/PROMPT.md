...
**Assessment:** Small extension with a `@sero-ai/ui` dependency (`cn` utility only) that needs to be inlined. Otherwise follows the standard extraction pattern.
...
**Source files to understand `@sero-ai/ui` usage:**
- `packages/pi-calc-extension/ui/CalcApp.tsx` — imports `cn` from `@sero-ai/ui/lib/utils`
...
- [ ] Read source files to catalogue all `@sero-ai/ui` imports (should be `cn` from `@sero-ai/ui/lib/utils` only)
...
- [ ] Replace all `@sero-ai/ui/lib/utils` imports with `./lib/utils` (or appropriate relative path)
- [ ] Remove all `@sero-ai/ui` references from the codebase
...
**Check If Affected:**
- `docs/plan-plugin-extraction.md` — update if `@sero-ai/ui` inlining pattern should be documented
...
- [ ] No `@sero-ai/ui` imports remain — `cn` utility is inlined
...
- Keep `@sero-ai/ui` as a dependency — inline what's needed
