...
const IRON_LIGHT = hex('6b7482');
const IRON = hex('444c59');
const IRON_DARK = hex('272d38');
...
export function buildCharacter(finish: Finish = RUST, dials: Dials = DEFAULT_DIALS): CharacterSpec {
  ...
  S.chain('antenna', 'head', [6, 46], 3, 9, [-260, 0], 320, 0.9, 0.55, 0.62, [0, 1]);
  ...
  const parts: Part[] = [
    ...
    { name: 'foot_far', bone: 'foot_far', ramp: iron, paint: foot(true) },
    ...
    { name: 'foot_near', bone: 'foot_near', ramp: iron, paint: foot(false) },
  ...
  const restPose = (): Pose => {
    const pose: Pose = { deg: {} };
    S.solveChain(pose, 'thigh_near', 'shin_near', [138, GROUND_Y], 1, 'foot_near', 90);
...
function thigh(finish: Finish, far: boolean): Paint {
  ...
  slab(p, 2, 46, far ? 10 : 12, 3.5, far ? IRON_DARK : IRON);
  if (!far) {
    p.tintToward([-1, -0.3], IRON_LIGHT, 3);
    p.tintToward([1, 0.3], IRON_DARK, 2.5);
  }
  // The hip pivot, a dark disc, so the joint reads as a hinge and not a seam.
  p.disc([0, 3], 8, far ? IRON_DARK : finish.shellDark);
...
function shin(finish: Finish, far: boolean): Paint {
  ...
  slab(p, 0, 30, far ? 9 : 11, 3, far ? IRON_DARK : IRON);
  // The boot: wider than the shin, which is what makes a robot look planted.
  slab(p, 28, 46, far ? 13 : 15, 4.5, far ? IRON_DARK : finish.shellDark);
  if (!far) {
    p.tintToward([-1, -0.3], finish.shellLight, 2);
    p.tintToward([1, 0.4], IRON_DARK, 2.5);
  }
  p.disc([0, 1], 7, far ? IRON_DARK : IRON_LIGHT);
  return p;
}
...
function foot(far: boolean): Paint {
  ...
  slab(p, -6, 17, 13, 4, far ? IRON_DARK : IRON);
  if (!far) p.tintToward([1, -0.3], IRON_LIGHT, 2);
  return p;
}
...
function hips(finish: Finish): Paint {
  ...
  slab(p, 0, 34, 22, 6, IRON);
  // The power belt, the one warm band low on the body.
  slab(p, 9, 18, 24, 3.5, finish.trimDark);
  slab(p, 12, 15, 25, 2, finish.trim);
  p.tintToward([0.9, 0.4], IRON_LIGHT, 2.5);
  p.tintToward([-0.9, -0.4], IRON_DARK, 2.5);
  return p;
}
...
function head(finish: Finish): Paint {
  ...
  const p = new Paint({ x: -44, y: -10, w: 88, h: 64 });
  ...
  p.polygon([[-33, 21], [-4, 23], [-4, 34], [-33, 31]], IRON_DARK);
  p.disc([-22, 27], 8, OPTIC);
  p.disc([-25, 28], 3.5, OPTIC_CORE);
  // The antenna socket, so the chain has something to grow out of.
  slab(p, 42, 52, 8, 2, IRON);
...
function forearm(finish: Finish, far: boolean): Paint {
  ...
  p.polygon([[-11, 22], [-3.5, 22], [-3.5, 36], [-11, 32]], far ? IRON_DARK : finish.shellDark);
  p.polygon([[11, 22], [3.5, 22], [3.5, 36], [11, 32]], far ? IRON_DARK : finish.shellDark);
  if (!far) p.tintToward([-1, -0.3], finish.shellLight, 2);
  return p;
}
...
function idle(dials: Dials): Motion {
  ...
  c.plant('thigh_near', 'shin_near', 'foot_near', { 0: [138, GROUND_Y, 90] });
...
function walk(dials: Dials): Motion {
  ...
  c.gait('thigh_near', 'shin_near', 'foot_near', dials.stride, 16, 0, GROUND_Y, 5, 0.68);
  c.gait('thigh_far', 'shin_far', 'foot_far', dials.stride, 16, 0.5, GROUND_Y, -5, 0.68);
...
function startle(dials: Dials): Motion {
  ...
  c.wobbleBudget = 3.5;
  c.wind = [dials.antennaWind * 1.6, 0];
  c.plant('thigh_near', 'shin_near', 'foot_near', {
    0: [138, GROUND_Y, 90],
    0.12: [140, GROUND_Y, 90],
    0.45: [136, GROUND_Y, 90],
    1.1: [138, GROUND_Y, 90],
  });
  c.plant('thigh_far', 'shin_far', 'foot_far', {
    0: [116, GROUND_Y, 88],
    0.12: [113, GROUND_Y, 88],
    0.45: [118, GROUND_Y, 88],
    1.1: [116, GROUND_Y, 88],
  });
  ...
  c.key('head', { 0: 0, 0.12: -12, 0.4: 6, 0.7: -2, 1.1: 0 });
