...
The agent loads the skill when you mention diagrams, architecture, flowcharts, schemas, or visualizations. It also kicks in automatically when it's about to dump a complex table in the terminal (4+ rows or 3+ columns) — it renders HTML instead and opens it in the browser. Output goes to `diagrams/`.
...
└── data-table.html   (tables with KPIs and badges — rose/cranberry palette)
    ↓
diagrams/filename.html → opens in browser
```
