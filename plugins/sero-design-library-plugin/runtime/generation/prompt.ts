...
function tweakRules(): string {
  const rules = [
    ...
    'After the baseline, add four to eight high-impact controls for the page-specific decisions worth revisiting. Cover at least three relevant style areas: Colour (background, surface, text or accent), Composition (content width, columns or hero height), Spacing (section rhythm, grid gap or density), Shape (radius, border or button form), and Atmosphere (shadow, contrast, texture or motion). A dense metrics dashboard may favour density, grid and signal colour; an editorial page may favour measure, rhythm and image treatment.',
    'Do not add a weak or duplicate control only to meet the count. Prefer controls that change the visual system across the page over controls that affect one small item.',
