...
import { mutateDesign, mutateVariant, saveDesign } from './design-store';
...
export async function retryVariant(
  ...
  let restarted = false;
  await mutateVariant(paths, designId, variantId, (variant) => {
    if (!RETRYABLE.includes(variant.status)) return null;
    restarted = true;
    ...
    return { ...variant, status: 'pending', error: undefined, jobId: undefined };
  });
  return restarted;
}
