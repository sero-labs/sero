 ...
 *
 * The panels below are separate components so each render branch is readable on
 * its own: the adjust panel, the recompute diff, the proposed team, and the
 * planner's reasoning.
 */
...
/** The free-text adjust panel. Controlled: the parent owns the pending check. */
function AdjustPanel({
  ...
  const append = (suggestion: string) =>
    onInstructionChange(instruction.trim() ? `${instruction.trim()} ${suggestion}.` : `${suggestion}.`);
    ...
    <section className="mt-4 overflow-hidden rounded-lg border border-status-info-border bg-linear-[160deg] from-status-info-faint to-transparent">
      ...
      <div className="p-4 pt-3">
        ...
        <div className="mt-3 flex justify-end gap-2">
          <Button variant="ghost" className="text-sm text-room-text3" onClick={onCancel} disabled={busy}>
            Cancel
          </Button>
...
/** The recompute diff: what the adjustment changed, against the old proposal. */
function DiffBand({ diff }: { diff: ReturnType<typeof proposalDiff> }) {
  return (
    <div className="mt-4">
      <AuthorityBand
        tone="neutral"
        ...
        footer={diffFooter(diff)}
      />
    </div>
  );
}

/** The kept/removed/added sentence the diff band carries. */
function diffFooter(diff: ReturnType<typeof proposalDiff>) {
  return (
    <>
      {diff.kept.length > 0 && (
...
/** The proposed team's member table (prototype frame 3). */
function ProposedTeam({ proposal }: { proposal: RoomProposalSummary }) {
  return (
    <section className="mt-6 overflow-hidden rounded-lg border border-room-line bg-room-surface">
      <div className="flex items-center border-b border-room-line px-4 py-3 text-sm font-medium text-room-text2">
        The team
        <span className="room-tabular ml-auto text-xs font-normal text-room-text3">{proposal.teamSize} members</span>
      </div>
...
/** The planner's own reasoning, which carries no authority. */
function WhyPanel({ proposal }: { proposal: RoomProposalSummary }) {
  return (
...
/** The team, the consent band and any access warnings — the unrevised proposal. */
function PlainProposal({ proposal }: { proposal: RoomProposalSummary }) {
  return (
    <>
      <ProposedTeam proposal={proposal} />
      <div className="mt-4">
        <AuthorityBand
          ...
          cells={[
            ...
            { label: 'Access', value: accessSentence(proposal.access) },
          ]}
        />
      </div>
...
/** The Room proposal's action row, including what designing the team cost. */
function ProposalFooter({
  ...
  return (
    <div className="mt-6 flex flex-wrap items-center gap-2.5">
      ...
      {!adjusting && (
        <Button variant="outline" className="h-9 px-4 text-sm" disabled={busy} onClick={onOpenAdjust}>
          Adjust
        </Button>
      )}
      ...
      <span className="ml-auto flex items-center gap-4">
        {revised && onDismissRevision && <DiscloseLink label="Full proposal" onClick={onDismissRevision} />}
        <DiscloseLink label={showWhy ? 'Hide reasoning' : 'Why this team?'} onClick={onToggleWhy} />
...
export function RoomProposal({
  ...
  return (
    <div className="mx-auto mt-5 flex w-full max-w-[61.5rem] flex-col px-6 pb-8">
      ...
      {diff && <DiffBand diff={diff} />}

      {!revised && <PlainProposal proposal={proposal} />}
      ...
      <ProposalFooter
        ...
        showWhy={showWhy}
        onStart={onStart}
        onOpenAdjust={() => setAdjusting(true)}
        onDiscard={onDiscard}
        onToggleWhy={() => setShowWhy((value) => !value)}
        onDismissRevision={onDismissRevision ? closeAdjust : undefined}
        onOpenAdvanced={onOpenAdvanced}
      />

      {showWhy && <WhyPanel proposal={proposal} />}
    </div>
  );
}
