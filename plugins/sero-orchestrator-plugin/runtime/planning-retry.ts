...
export async function runPlanningWithRetry(host: OrchestratorHost, params: ModelRunParams): Promise<ModelRunResult> {
  if (params.platformTools !== 'none') throw new Error('Planning retries require a tool-free call.');
  for (let attempt = 0; ; attempt += 1) {
    params.signal?.throwIfAborted();
    const result = await host.runStructured(params).catch((error: unknown): ModelRunResult => ({
