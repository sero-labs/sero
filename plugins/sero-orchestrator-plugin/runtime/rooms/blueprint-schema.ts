...
/**
 * The shape asked for in the prompt. Kept next to the parser so the two cannot
 * drift: a field described here but not read there is silently dropped, and the
 * reverse is a guaranteed repair pass.
 */
export const ROOM_BLUEPRINT_JSON_SHAPE = `{
  ...
  "deliveryDestination": string,         // one id from the envelope's allowedDeliveryDestinations
