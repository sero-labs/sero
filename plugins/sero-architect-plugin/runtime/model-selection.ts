import { type OrchestratorProjectModelSnapshot, modelKey } from '@sero-ai/common';
import type { ArchitectHost } from './host';

/** Keep a delegated Room inside the model choices the user made in Admin. */
export async function roomModelLimits(host: Pick<ArchitectHost, 'modelTiers'>, snapshot?: OrchestratorProjectModelSnapshot) {
  const tiers = Object.values(snapshot ?? await host.modelTiers());
