...
describe('memory tool CRUD semantics', () => {
  ...
  it('preserves entry ids across replace/remove and blocks duplicate writes', async () => {
    ...
    const secondWrite = await handleWrite(root, 'memory', 'The release checklist lives in docs/releases.md.', 'append', 'decision');
    ...
    const readWithIds = await handleRead(root, 'memory', true);
  ...
  it('enforces the MEMORY.md visible-capacity limit before writing', async () => {
    ...
    expect(getText(writeResult)).toMatch(
      /^Error: MEMORY\.md would exceed capacity \(\d+\/4000 chars\)\. Current usage: 100%\. Replace or remove content before adding more\.$/,
    );
