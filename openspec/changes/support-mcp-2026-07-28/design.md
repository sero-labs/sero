...
- **Viewer server.** `ui-server.ts` holds a map of sessions keyed by a random token, with a limit on concurrent sessions and teardown on close. It no longer holds one global session. The shell loads from `127.0.0.1:<random port>`, separate from the Sero UI origin. Each app frame loads from its own `<label>.localhost:<port>` host, so it keeps a real origin without sharing one with the shell or with another app, and the app URL carries no shell token. The shell CSP allows that app origin in `frame-src`. The app frame keeps `allow-same-origin` because a real origin is what makes the camera and microphone permissions the user approves usable. The desktop CSP hook keeps a framed page's own policy for loopback hosts, and it counts `.localhost` names as loopback, so each app keeps its default-deny policy.
