...
- `0` means a measured zero. In the tiles and charts, a value that was not measured reads `unavailable`. In the selected-activity panel, a fact that was not recorded is left out.
- Charges without per-call detail from one kind of source are one row per activity, such as `Room spend · 62 updates`.
- Active time includes the working time Rooms and Workflows report, and time ranges show dates when they cross a day.
...
| Zoom in, Zoom out | Kept as named icon buttons, plus **Zoom to selection** and the draggable overview window |
...
- `architect-run-observability`: adds requirements for named activity, owner wakes as operations with model and tokens, loading activity on open, merged running-total charges, delegated working time in Active, dated time ranges, unmeasured values versus zero, one authoritative cost, and empty filter results.
...
- `@sero-ai/common` 0.18.1: `OrchestratorBoardRoomView` gains the optional `activeMs` and `activeSince`, which the Orchestrator's Room summary fills from the Room record. No host or IPC changes.
- `dispatch-watch.ts`, `research-room.ts` and `research-workflow.ts` record the rise in a Room's or Workflow's working time beside its cost rise.
