...
| `docs/superpowers/specs/2026-04-04-dynamic-model-provider-design.md` | historical feature design spec | durable internal | code, `docs/decisions.md` | retain as the canonical architecture record for shipped dynamic model provider work |
| `docs/superpowers/specs/2026-04-04-onboarding-resilience-analysis.md` | historical failure analysis / design note | durable internal | onboarding/auth resilience follow-up work | retain as durable internal root-cause context |
| `docs/superpowers/specs/**` | higher-fidelity design/spec docs | durable internal if still referenced; otherwise transient with reusable facts | `docs/features/**`, `docs/architecture.md`, `docs/decisions.md` | tail triage complete for now; only the two retained durable internal records remain |
