...
import type { Card, KanbanSettings } from './types';
...
export async function executeLightReview(
  ...
  const compileCommands = await detectCompileCommands(worktreePath);
  if (compileCommands.length > 0) {
    tracker.setPhase('Running compile checks');
    await tracker.flush();

    const compileResult = await runVerificationCommands(worktreePath, compileCommands, undefined, {
  ...
  const devServerCommand = await detectDevServerCommand(worktreePath);
  if (devServerCommand) {
    tracker.setPhase('Checking dev server startup');
    await tracker.flush();

    const devServerResult = await runDevServerSmokeCheck(worktreePath, devServerCommand, {
      runCommand: (command, cwd, timeoutMs) =>
        runWorkspaceCommand(deps.workspaceId, cwd, command, timeoutMs, { isolated: true }),
    });
    if (!devServerResult.success) {
      return {
        success: false,
        error: `Light review dev server check failed:\n${summarizeVerificationFailure(devServerResult)}`,
      };
    }
