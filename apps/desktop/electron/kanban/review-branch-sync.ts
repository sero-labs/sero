...
import { buildConflictResolutionPrompt } from './prompt-conflict-resolution';
import { syncWorktreeBranchWithDefaultBranch } from './worktree-sync';
...
export async function syncReviewBranchWithDefault(
  ...
  const result = await syncWorktreeBranchWithDefaultBranch(worktreePath, {
    resolveConflicts: async ({ attempt, baseBranch, conflictFiles }) => {
      ...
      const resolution = await deps.subagentManager.runSingleStructured({
        agent: 'implementer',
