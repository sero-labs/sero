...
export async function resolvePreferredBaseRef(workspacePath: string): Promise<string | null> {
  await fetchOrigin(workspacePath, defaultWorktreeRunner);

  const branch = await detectDefaultBranch(workspacePath, defaultWorktreeRunner);
  if (!branch) return null;

  if (await refExists(workspacePath, `refs/remotes/origin/${branch}`, defaultWorktreeRunner)) {
    return `origin/${branch}`;
  }
  if (await refExists(workspacePath, `refs/heads/${branch}`, defaultWorktreeRunner)) {
    return branch;
  }
...
async function fetchOrigin(workspacePath: string, runner: WorktreeGitRunner): Promise<void> {
  try {
...
async function resolveRef(workspacePath: string, ref: string, runner: WorktreeGitRunner): Promise<string | null> {
  try {
...
async function getMeaningfulWorkspaceStatusPaths(workspacePath: string, runner: WorktreeGitRunner): Promise<string[]> {
  try {
