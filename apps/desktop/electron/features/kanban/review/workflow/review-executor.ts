...
import type { ReviewIssue, ReviewResult, ReviewPromptOptions } from '@electron/features/kanban/prompts';
import {
  buildReviewPrompt,
...
async function runFreshReviewPipeline(
  ...
  for (let revisionPass = 0; revisionPass <= MAX_CRITICAL_REVISIONS; revisionPass++) {
    ...
    const reviewInputs = await prepareReviewInputs(worktreePath, card, tracker, expectedPaths);
