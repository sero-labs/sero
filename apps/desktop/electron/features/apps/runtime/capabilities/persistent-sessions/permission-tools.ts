...
export function applyPermissionProfile(
  ...
  const permitted = (tool: string): boolean => {
    ...
    if (matches(tool, COMMAND_TOOLS)) return profile.commands !== 'none';
    ...
    // Everything else is a read or a Room-protocol tool. `filesystem: 'none'`
    // is a session that cannot even read, which is only useful for a pure
    // reasoning member.
    return profile.filesystem !== 'none';
  };
