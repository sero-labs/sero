...
/** Activate the Explorer app so MultiRootFileTree mounts. */
async function activateExplorer(page: Page) {
  await page.evaluate(() => {
    (window as any).__appControl?.openApp('explorer');
  });
  // Allow React to re-render with the new active app
