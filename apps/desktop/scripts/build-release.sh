...
rm -rf "$DEPLOY_DIR"
cd "$MONO_ROOT"
# inject-workspace-packages: keeps the deploy bundle shape that pnpm v10's
# lockfile-backed deploy required. pnpm 12 no longer requires it, but without it
# deploy binds peers differently. Keep this scoped to release packaging instead
# of changing the workspace install layout repo-wide.
# pnpm 12 reads settings from pnpm_config_* variables, not npm_config_*.
