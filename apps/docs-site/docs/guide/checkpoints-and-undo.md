...
Sero has more than one recovery tool. Manual checkpoints restore workspace files. Chat turn undo restores files and rewinds the session tree for a specific agent turn. Git source-control operations are separate repository operations.
