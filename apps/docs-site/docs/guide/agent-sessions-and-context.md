...
```mermaid
flowchart TD
  UserPrompt[User prompt] --> Composer[Chat composer]
  Files["@ file references"] --> Composer
  Snapshot[Workspace snapshot] --> Composer
  Attachments[Attachments] --> Composer
  Context[Session context editor] --> Session[Session config]
  Memory[Selected memory context] --> Payload[Turn payload]
  Composer --> Payload
  Session --> Payload
  Payload --> Model["Selected model/provider"]
  Model --> Pi[Pi session runtime]
  Pi --> Tools[Tools and commands]
  Pi --> Response[Streaming response]
  Response --> Queue{Queued follow-up?}
  Queue -->|Yes| Composer
```

![Agent context flow](../assets/generated/img3.jpg)
