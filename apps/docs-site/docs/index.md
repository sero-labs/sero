...
features:
  ...
  - title: Container-backed when available
    details: Apple Container and Docker-backed workspaces are the preferred runtime for isolation, tooling, and Linux parity, with host mode available as a reduced fallback where supported.
...
- fall back to host mode on macOS/Linux when container runtimes are unavailable
