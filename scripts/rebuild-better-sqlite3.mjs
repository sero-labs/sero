...
function installElectronBinary(electronDir) {
  if (process.env.ELECTRON_SKIP_BINARY_DOWNLOAD) {
    console.error('[better-sqlite3] ELECTRON_SKIP_BINARY_DOWNLOAD is set, so Electron cannot download its runtime binary. Unset it or set SERO_SKIP_NATIVE_REBUILD=1.');
    return null;
  }
  ...
  console.log('[better-sqlite3] Electron package exists but binary is missing; downloading Electron...');
  try {
    ...
    console.error(`[better-sqlite3] Electron binary download failed: ${error.message}`);
    return null;
  }
  ...
  if (process.platform === 'darwin') {
    ...
    if (archive) {
      ...
      try {
        ...
        console.warn(`[better-sqlite3] Could not restore Electron from cache: ${error.message}`);
      }
    }
  }
