...
export interface LspServerConfig {
  ...
  language: string;
  /** Command to run inside the container (e.g. 'typescript-language-server --stdio') */
  command: string;
  ...
  /** LSP language IDs for didOpen (extension → lsp language id) */
...
/** Find a server config by file extension */
export function findConfigByExtension(ext: string): LspServerConfig | undefined {
  return LANGUAGE_SERVERS.find(s => s.extensions.includes(ext));
}
...
export function getLspLanguageId(config: LspServerConfig, filePath: string): string {
  ...
  return config.languageIdMap[ext] ?? config.language;
}
...
/** Convert a file:// URI back to a container path */
