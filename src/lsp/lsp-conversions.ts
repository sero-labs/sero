...
// Monaco CompletionItemKind values (stable public API)
const MONACO_KIND = {
...
const LSP_TO_MONACO_KIND: Record<number, number> = {
  1: MONACO_KIND.Text,         // Text
  2: MONACO_KIND.Method,       // Method
  3: MONACO_KIND.Function,     // Function
  4: MONACO_KIND.Constructor,  // Constructor
  5: MONACO_KIND.Field,        // Field
  6: MONACO_KIND.Variable,     // Variable
  7: MONACO_KIND.Class,        // Class
  8: MONACO_KIND.Interface,    // Interface
  9: MONACO_KIND.Module,       // Module
  10: MONACO_KIND.Property,    // Property
  11: MONACO_KIND.Unit,        // Unit
  12: MONACO_KIND.Value,       // Value
  13: MONACO_KIND.Enum,        // Enum
  14: MONACO_KIND.Keyword,     // Keyword
  15: MONACO_KIND.Snippet,     // Snippet
  16: MONACO_KIND.Color,       // Color
  17: MONACO_KIND.File,        // File
  18: MONACO_KIND.Reference,   // Reference
  19: MONACO_KIND.Folder,      // Folder
  20: MONACO_KIND.EnumMember,  // EnumMember
  21: MONACO_KIND.Constant,    // Constant
  22: MONACO_KIND.Struct,      // Struct
  23: MONACO_KIND.Event,       // Event
  24: MONACO_KIND.Operator,    // Operator
  25: MONACO_KIND.TypeParameter, // TypeParameter
};
...
const LSP_TO_MONACO_SEVERITY: Record<number, number> = {
  1: MARKER_SEVERITY.Error,
  2: MARKER_SEVERITY.Warning,
  3: MARKER_SEVERITY.Info,
  4: MARKER_SEVERITY.Hint,
};

// Monaco CompletionItemInsertTextRule
const INSERT_TEXT_RULE = { InsertAsSnippet: 4 };

/** Convert LSP position (0-indexed) to Monaco position (1-indexed) */
export function lspPosToMonaco(pos: { line: number; character: number }) {
...
export function convertCompletions(
  ...
  const suggestions = items.map((item: any) => {
    ...
    // Handle snippet insert format
    if (item.insertTextFormat === 2) {
      suggestion.insertTextRules = INSERT_TEXT_RULE.InsertAsSnippet;
    }
    ...
    if (item.textEdit) {
      ...
      if (edit.range) {
        suggestion.range = lspRangeToMonaco(edit.range);
      }
      suggestion.insertText = edit.newText;
    }

    if (item.deprecated || item.tags?.includes(1)) {
      suggestion.tags = [1]; // Monaco CompletionItemTag.Deprecated
    }
...
export function isLspSupported(languageId: string): boolean {
  // Monaco uses 'typescript' for both .ts and .tsx — check that
  return languageId === 'typescript' || languageId === 'javascript' || LSP_LANGUAGES.includes(languageId);
}

/** Get the normalized LSP server language key for a Monaco language ID */
