...
export function FileTreeContextMenu({
  ...
  const handleDelete = useCallback(async () => {
    if (isRoot) return;
    const ok = await deleteItem(projectId, itemPath);
    if (ok) onDeleted?.(itemPath);
  }, [projectId, itemPath, isRoot, onDeleted]);
  ...
  const handleNewItemSubmit = useCallback(async () => {
    ...
    const ok = newItemMode === 'file'
      ...
      : await createFolder(projectId, fullPath);

    if (ok) onReloadDir(targetDir);
    resetNewItem();
  }, [newItemName, newItemMode, targetDir, projectId, onReloadDir, resetNewItem]);
        ...
        {newItemMode ? (
          <div className="px-2 py-1.5">
            <Input
              ref={inputRef}
              autoFocus
              className="-my-0.5 h-7 px-2 text-sm"
